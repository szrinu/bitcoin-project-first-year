...
y_overall = maindf.loc[(maindf['Date'] >= '2014-09-21')
                     & (maindf['Date'] <= '2024-02-06')]

y_overall.drop(y_overall[['Adj Close','Volume']],axis=1)
print(maindf.head())
maindf['Open-Close']  = maindf['Open'] - maindf['Close']
maindf['Low-High']  = maindf['Low'] - maindf['High']
maindf['Target'] = np.where(maindf['Close'].shift(-1) > maindf['Close'], 0, 1)
plt.pie(maindf['Target'].value_counts().values, 
...
fig.update_yaxes(showgrid=False)
fig.show()
closedf = closedf[closedf['Date'] > '2023-02-06']
